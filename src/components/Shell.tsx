...
import { isFinanceAllowed } from '@/lib/finance';
...
type MetaKey = 'home'|'tickets'|'ticketNew'|'ticket'|'leads'|'cases'|'clients'|'services'|'agenda'|'news'|'config'|'finance';
const META: Record<MetaKey, { eyebrow:string; title:string; icon:string; cta:string; ctaHref?:string }> = {
  ...
  finance:   { eyebrow:'Financeiro',   title:'Financeiro IA · DataDocs', icon:'account_balance', cta:'Nova base', ctaHref:'/financeiro' }
};
...
export async function Shell({ active, children }: { active: MetaKey; children: React.ReactNode }) {
  ...
  const canFinance = await isFinanceAllowed();
  ...
  return (
    <div className="min-h-screen text-ink-100 font-sans" style={{ display:'grid', gridTemplateColumns:'236px 1fr', gridTemplateRows:'64px 1fr' }}>
      <aside className="row-span-2 flex flex-col gap-1 border-r border-[rgba(212,175,55,.15)] bg-sidebar-gradient p-4">
        ...
        {canFinance && (
          <>
            <div className="px-2 pt-4 pb-1 text-[10px] font-semibold uppercase tracking-[.24em] text-ink-500">Financeiro</div>
            <NavItem href="/financeiro" label="Financeiro IA" icon="account_balance" active={active === 'finance'} lock />
          </>
        )}
...
function NavItem({ href, label, icon, active, badge, lock }: { href:string; label:string; icon:string; active?:boolean; badge?:string; lock?:boolean }) {
  const cls = active
  ...
  return (
    <Link href={href} className={`flex items-center gap-3 rounded-lg border px-2.5 py-2 text-sm ${cls}`}>
      ...
      <span>{label}</span>
      {lock && <Icon name="lock" size={14} className="ml-auto text-gold-300/70" />}
